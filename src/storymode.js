...
import { maybeAccountLink, gameplayStart, gameplayStop } from "./sdk.js";
...
export function createStoryMode(deps) {
  ...
  function measureBookCover() {
    const journal = el("journal");
    const shelf = el("shelf");
    const sub = el("story-sub");
    const h2 = el("story-h2");
    const journalWasHidden = journal.classList.contains("hidden");
    const shelfWasHidden = shelf.classList.contains("hidden");
    const subWasHidden = sub.classList.contains("hidden");
    const h2Text = h2.textContent;
    jrIndex = Math.min(storyProgress(), CHAPTERS.length - 1);
    renderJournal();
    h2.textContent = t("storyTitle");
    journal.style.visibility = "hidden";
    journal.classList.remove("hidden");
    shelf.classList.add("hidden");
    sub.classList.remove("hidden");
    const jrBook = el("jr-book");
    jrBook.classList.add("snap", "closed");
    void jrBook.offsetWidth;
    const cover = jrBook.querySelector(".jr-half-right");
    const r = (cover !== null ? cover : jrBook).getBoundingClientRect();
    jrBook.classList.remove("snap", "closed");
    if (journalWasHidden) {
  ...
  el("shelf-ep1").addEventListener("click", function () {
    ...
    const curTransform = getComputedStyle(book).transform;
    const curMatrix = curTransform && curTransform !== "none" ? new DOMMatrixReadOnly(curTransform) : new DOMMatrixReadOnly();
    const curX = curMatrix.m41;
    const curY = curMatrix.m42;
    const rect = book.getBoundingClientRect();
    book.style.transition = "none";
    book.style.transform = "translate(" + curX + "px, " + curY + "px) scale(1)";
    void book.offsetWidth;
    const visualScale = rect.width / book.offsetWidth;

    const target = measureBookCover();
    const originCx = rect.left + rect.width / 2 - curX * visualScale;
    const originCy = rect.top + rect.height / 2 - curY * visualScale;
    const dx = (target.cx - originCx) / visualScale;
    const dy = (target.cy - originCy) / visualScale;
