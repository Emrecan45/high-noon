...
import { restoreStoryBackup, restoreStoryFromMask, storyProgress, CHAPTERS } from "./story.js";
...
function layoutStage() {
  ...
  const scale = Math.min(vh / DESIGN_H, vw / MIN_DESIGN_W);
...
const MUSIC_MID = 0.6;
const SFX_MID = 0.9;
const sensSlider = el("sens-range");
function renderSetValue(node, slider) {
  el(node).textContent = slider.value + "%";
}
...
sensSlider.value = String(getMouseSensPct());
renderSetValue("music-value", musicSlider);
renderSetValue("sfx-value", sfxSlider);
renderSetValue("sens-value", sensSlider);
...
sensSlider.addEventListener("input", function () {
  setMouseSensPct(Number(sensSlider.value));
  renderSetValue("sens-value", sensSlider);
});
...
el("btn-community").addEventListener("click", function () {
  ...
  showPopup(t("communityTitle"), "<p>" + t("communitySoon") + "</p>");
});
...
el("settings-close").addEventListener("click", function () {
  audio.uiClick();
...
function itemPopupIcon(item) {
  if (!item.icon) {
    return "";
  }
  return '<img class="pop-item-ico rarity-' + item.rarity + '" src="' + item.icon + '" alt="" />';
}

function showItemWonPopup(item) {
  showPopup(t("wheelWinTitle"), itemPopupIcon(item) + "<p>" + t("wheelNew", { item: t(item.nameKey) }) + "</p>");
}
...
function caseItemFor(kind, ref) {
  ...
  if (kind === "skin") {
    const skin = skinById(ref);
    if (skin.id === ref) {
      return { kind: kind, ref: ref, nameKey: skin.nameKey, icon: portraitDataUrl(skin.id, 64), rarity: skinRarity(skin) };
    }
  }
  if (kind === "weapon") {
    const weapon = weaponById(ref);
    if (weapon.id === ref) {
      return { kind: kind, ref: ref, nameKey: weapon.nameKey, icon: weaponIconDataUrl(weapon.id, 64), rarity: rarityOf(weapon.price) };
    }
  }
  if (kind === "accessory") {
    const acc = accessoryById(ref);
    if (acc !== null && acc.id === ref && acc.nameKey !== null) {
      return { kind: kind, ref: ref, nameKey: acc.nameKey, icon: accessoryIconDataUrl(acc.id, 64), rarity: accessoryRarity(acc.id) };
    }
  }
  return caseItems[0];
}
...
function adItemCell(item) {
  ...
  cell.className = "reward-cell rarity-" + info.rarity;
  const img = document.createElement("img");
  img.src = info.icon;
  const label = document.createElement("div");
  label.className = "reward-count";
  let go = null;
  if (item.owned) {
    label.textContent = "✔";
...
const AD_ITEMS = [
  { kind: "accessory", ref: "cigarette", needed: 3 },
  { kind: "weapon", ref: "peacemaker", needed: 5 },
  { kind: "accessory", ref: "paper-torn", needed: 6 },
  { kind: "skin", ref: "trapper", needed: 10 }
];

function adItemList() {
  if (adShopData !== null && Array.isArray(adShopData.items) && adShopData.items.length > 0) {
    return adShopData.items;
  }
  return AD_ITEMS.map(function (item) {
    return { kind: item.kind, ref: item.ref, needed: item.needed, watched: 0, owned: false };
  });
}

function renderAdItems() {
  const row = el("reward-items");
  row.innerHTML = "";
  for (const item of adItemList()) {
    row.appendChild(adItemCell(item));
  }
  const btn = el("btn-crate-watch");
  if (adCaseTimer !== null) {
    clearInterval(adCaseTimer);
    adCaseTimer = null;
  }
  if (adShopData !== null && adShopData.case_left <= 0) {
    btn.disabled = true;
...
el("btn-crate-watch").addEventListener("click", function () {
  ...
  requestRewardedAd({
    ...
    onFinish: function () {
      ...
      adCase().then(function (result) {
        ...
        el("btn-crate-watch").disabled = true;
        audio.whoosh();
        runCaseAnimation(result, function () {
          ...
          if (result.duplicate) {
            audio.coin();
            showPopup(t("wheelDupTitle"), itemPopupIcon(item) + "<p>" + t("wheelDup", { item: t(item.nameKey), n: result.refund }) + "</p>");
          } else {
            audio.wheelWin();
            showItemWonPopup(item);
          }
        });
      });
    },
...
el("btn-spin").addEventListener("click", async function () {
  ...
  if (!result.ok) {
    if (result.reason === "poor") {
      showPopup(t("wheelPoorTitle"), "<p>" + t("wheelPoor") + "</p>");
    } else {
      showPopup(t("wheelPoorTitle"), "<p>" + t("accountError") + "</p>");
    }
    return;
  }
  spinning = true;
...
el("btn-home-reward").addEventListener("click", function () {
  bootAudio();
  accountReady().then(function (profile) {
    if (profile === null) {
      showToast(t("shopAdFail"), null, null, 4000);
    ...
    el("btn-home-reward").disabled = true;
    requestRewardedAd({
      onStart: adMuteOn,
      onFinish: function () {
        adMuteOff();
        el("btn-home-reward").disabled = false;
...
async function refreshFriends() {
  ...
  if (rows !== null) {
    internal = rows;
  } else if (Array.isArray(friendsCache)) {
    internal = friendsCache.filter(function (f) {
      return !f.cg;
    });
  }
  let merged = internal;
  if (isRealCrazyGames()) {
    ...
    if (cg !== null) {
      merged = mergeFriends(cg, internal);
    }
  }
  friendsCache = merged;
  renderFriends();
...
function mergeFriends(cg, internal) {
  const ids = new Set();
  ...
  const out = internal.slice();
  for (const f of cg) {
    if (f.profileId !== null && ids.has(f.profileId)) {
      continue;
    }
    out.push(f);
  }
...
function isAlreadyFriend(oppId) {
  if (oppId === null || !Array.isArray(friendsCache)) {
    return false;
  }
  for (const f of friendsCache) {
    if (f.cg) {
      if (f.profileId === oppId) {
        return true;
      }
    } else if (f.id === oppId) {
      return true;
...
function cgBadge() {
  const img = document.createElement("img");
  img.className = "friend-cg-badge";
  img.src = "https://www.google.com/s2/favicons?domain=crazygames.com&sz=64";
  img.alt = "CG";
  img.title = t("friendsCgTag");
  return img;
}
...
function duelProfile() {
  const profile = getProfile();
  if (profile === null) {
    return { id: null, pseudo: localPseudo(), skin: "drifter", acc: [], weapon: "iron", prime: 100, friend_code: null };
  }
  return { id: profile.id, pseudo: profile.pseudo, skin: profile.skin, acc: profile.accessories, weapon: profile.weapon, prime: profile.prime, friend_code: profile.friend_code || null };
}
...
function handleResult(ranked) {
  return function (won, oppPrime, stats, oppId) {
    ...
    return reportResult(won, ranked, oppPrime, oppId).then(function (result) {
      if (result === null) {
        return "";
      }
      if (won) {
...
function onSalonMatchEnd(oppId, oppName, oppCode) {
  const btn = el("btn-matchend-friend");
  if (btn === null) {
    return;
  }
  btn.classList.add("hidden");
  btn.onclick = null;
  if (!oppCode || getProfile() === null || !netAvailable()) {
    return;
  }
  if (oppId !== null && getProfile().id === oppId) {
    return;
  }
  if (isAlreadyFriend(oppId)) {
    return;
  }
  btn.disabled = false;
  btn.textContent = t("friendsAddOpp", { name: oppName });
  btn.classList.remove("hidden");
  btn.onclick = async function () {
    btn.disabled = true;
    const res = await sendFriendRequest(oppCode);
    if (res.ok) {
      btn.textContent = t("friendsSent");
      audio.friendPing();
      if (res.target) {
        notifyFriendsChange(res.target);
      }
      refreshFriends();
    } else if (res.reason === "already") {
      btn.textContent = t("friendsAlready");
    } else {
      btn.textContent = t("friendsAddOpp", { name: oppName });
      btn.disabled = false;
    }
...
function startNetDuel(room, ranked, friendly) {
  ...
  activeDuel = new Duel({
    ...
    onCombat: startCombatMusic,
    friendly: friendly,
    onMatchEnd: friendly ? onSalonMatchEnd : null,
...
async function generateRankedBot() {
  ...
  do {
    ...
    pseudo = "User" + idStr;
  } while (profile !== null && profile.pseudo === pseudo);
...
function openFriendRoom(rawCode, hosting, isChallenge = false, oppName) {
  ...
  friendRoom = createPrivateRoom(code, {
    ...
    onEmpty: function (sawPartner) {
      ...
      stopSearch();
      leaveStation();
      showPopup(t("salonGoneTitle"), "<p>" + t(sawPartner ? "salonLeftBody" : "salonGoneBody") + "</p>");
    }
  }, roomPid, !hosting);
  if (roomWaitTimer !== null) {
    clearTimeout(roomWaitTimer);
...
el("btn-salon").addEventListener("click", async function () {
  bootAudio();
  const profile = await accountReady();
  if (profile === null) {
    alert(t("connectError"));
...
function seasonDaysLeft() {
  if (seasonData !== null && Number.isFinite(Number(seasonData.days_left))) {
    return Number(seasonData.days_left);
  }
...
function runMenuGuide() {
  ...
  skip.className = "btn btn-small guide-skip";
  ...
  function show() {
    ...
    const r = { left: cr.left / scale, top: cr.top / scale, right: cr.right / scale, width: cr.width / scale, height: cr.height / scale };
    ...
    skip.textContent = t("guideSkip");
    skip.classList.toggle("hidden", i === steps.length - 1);
    const bw = 260;
    let bx = r.right + 18;
    if (bx + bw > vw - 12) {
...
async function boot() {
  ...
  loadingStart();
  audio.setMuted(cgAudioMuted());
  onCgSettingsChange(function (settings) {
    audio.setMuted(settings !== null && settings !== undefined && settings.muteAudio === true);
  });
  onCgAuthChange(function () {
    syncCgAccount();
  });
  onCgRoomJoin(function (params) {
    if (params === null || params === undefined || !netAvailable()) {
      return;
    }
    const code = params.roomId;
    if (typeof code !== "string" || code === "" || code === friendCode) {
      return;
    }
    accountReady();
    openFriendRoom(code, false);
  ...
  const joinKey = joinCode !== null && joinCode !== "" ? "hn-boot-joined-" + joinCode : null;
  let joinedBefore = false;
  try {
    joinedBefore = joinKey !== null && sessionStorage.getItem(joinKey) !== null;
  } catch (e) {}
  if (joinKey !== null && !joinedBefore && netAvailable()) {
    try { sessionStorage.setItem(joinKey, "1"); } catch (e) {}
