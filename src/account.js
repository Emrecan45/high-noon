...
export async function markItemSeen(kind, id) {
  ...
  if (kind === "skin") {
    ...
    await supabase.from("profile_skins").update({ seen: true }).eq("profile_id", profile.id).eq("skin_id", id);
  } else if (kind === "accessory") {
    unseenAcc.delete(id);
    await supabase.from("profile_accessories").update({ seen: true }).eq("profile_id", profile.id).eq("accessory_id", id);
  } else if (kind === "weapon") {
    unseenWeapons.delete(id);
    await supabase.from("profile_weapons").update({ seen: true }).eq("profile_id", profile.id).eq("weapon_id", id);
  }
}
...
async function fetchProfile() {
  ...
  if (skinRows !== null) {
    for (const row of skinRows) {
      ...
      if (!row.seen) unseenSkins.add(row.skin_id);
    }
  }
  ...
  if (accRows !== null) {
    for (const row of accRows) {
      ...
      if (!row.seen) unseenAcc.add(row.accessory_id);
    }
  }
  ...
  if (weaponRows !== null) {
    for (const row of weaponRows) {
      ...
      if (!row.seen) unseenWeapons.add(row.weapon_id);
    }
  }
}
