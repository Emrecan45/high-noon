...
export function wasProfileCreated() {
...
export async function bindCgAccount() {
  ...
  try {
    ...
    if (check.data.session !== null) {
      backupSession(check.data.session);
    }
    return true;
  } catch (err) {
    return false;
...
export async function reportResult(won, ranked, oppPrime, oppId) {
  ...
  profile.wins = Number(profile.wins || 0) + (won ? 1 : 0);
  profile.losses = Number(profile.losses || 0) + (won ? 0 : 1);
  if (ranked) {
    profile.ranked_wins = Number(profile.ranked_wins || 0) + (won ? 1 : 0);
    profile.ranked_losses = Number(profile.ranked_losses || 0) + (won ? 0 : 1);
  }
  return data;
}
