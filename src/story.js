...
export function restoreStoryFromMask(mask) {
  const value = Number(mask);
  if (!Number.isFinite(value) || value <= 0) {
    return;
  }
  let done = 0;
  while (done < CHAPTERS.length && (value & (1 << done)) !== 0) {
    done += 1;
  }
  if (done > storyProgress()) {
    localStorage.setItem("hn-story2", String(done));
    cgDataSet("hn-story2", String(done));
  }
}
